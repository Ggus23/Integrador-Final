...
NOMBRES = [
    "Valeria Quispe", "Fernanda Flores", "Camila Mamani", "Gabriela Condori",
    "Natalia Rojas", "Andrea Vargas", "Sofia Castro", "Luciana Ramos",
    "Daniela Torrico", "Mariana Siles", "Claudia Mendoza", "Alejandra Ortiz",
    "Isabela Guzman", "Valentina Rocha", "Paulina Camacho", "Lucía Carvajal",
    "Elena Miranda", "Carmen Gutierrez", "Teresa Villarroel", "Patricia Suarez",
    "Adriana Pinto", "Beatriz Blanco", "Silvia Marin", "Gloria Herrera",
    "Alicia Medina", "Alejandro Gómez", "Carlos Mamani", "Sebastian Flores",
    "Mateo Quispe", "Javier Condori", "Mauricio Rojas", "Rodrigo Vargas",
    "Daniel Castro", "Nicolas Ramos", "Gabriel Torrico", "Diego Siles",
    "Hugo Mendoza", "Adrian Ortiz", "Fernando Guzman", "Bruno Rocha",
    "Alvaro Camacho", "Andres Carvajal", "Jose Miranda", "Luis Gutierrez",
    "Ramiro Villarroel", "Mario Suarez", "Ricardo Pinto", "Jorge Blanco",
    "Walter Marin", "Oscar Herrera", "Ana Beltran", "Luisina Ponce",
    "Rosa Barrientos", "Pedro Castillo", "Santiago Lima", "Iván Morales",
    "Paola Céspedes", "Ronald Vargas", "Diego Peredo", "Carla Arce",
    "Marcelo Ríos", "Ximena Parada", "Felipe Vargas", "Lorena Suárez",
    "Pablo Cortés", "Natalia Paz", "Andrés Soliz", "María Roca",
    "Tomás Cardozo", "Regina Ferrufino", "Jorge Antezana", "Cecilia Landívar",
    "Emilio Cabrera", "Daniela Ribera", "Víctor Hugo Vaca", "Ángeles Pinto",
    "Gustavo Navarro", "Sandra Velasco", "Ramiro Vargas", "Soledad Durán",
    "Fabricio Morales", "Julia Campos", "Oliver Saavedra", "Tatiana Vargas",
    "Mónica Ballón", "Eduardo Hurtado", "Lourdes Aguilar", "Ignacio Pereira",
    "Fabiola Guzmán", "Federico López", "Helena Arteaga", "Maximiliano García",
    "Bárbara Ríos", "Emilio Vaca", "Graciela Toro", "Luis Fernando Campos",
    "Ruth Castellón", "Milton Rojas", "Carmen Rosa Panozo", "Diego Fernández",
    "Rocío Rocha", "Víctor Jiménez", "Yesenia Vargas", "Juan Carlos Pinto",
]
...
SUBJECTS_BY_COURSE = {
    "Ingeniería en Sistemas": [
        "Introducción a la Programación", "Cálculo I", "Álgebra Lineal",
        "Estructura de Datos", "Base de Datos I", "Análisis y Diseño de Sistemas",
        "Sistemas Operativos", "Física I"
    ],
    "Psicología": [
        "Introducción a la Psicología", "Neuroanatomía", "Psicología del Desarrollo",
        "Psicobiología", "Psicología Clínica", "Psicopatología I",
        "Métodos de Investigación", "Procesos Cognitivos"
    ],
    "Medicina": [
        "Anatomía Humana", "Histología", "Bioquímica Médica", "Fisiología I",
        "Embriología", "Patología General", "Farmacología Básica", "Semiología Médica"
    ]
}

EMOTION_COLORS = {
    "feliz": "#10b981", "motivado": "#8b5cf6", "neutral": "#6b7280",
    "ansioso": "#f59e0b", "triste": "#3b82f6", "frustrado": "#ef4444"
}
...
def create_student(db: Session, idx: int, risk_label: str, names: list):
    ...
    consent = Consent(user_id=user.id, has_accepted=True, version="1.0",
    ...
    def hito_procesual(gpa):
        return int(max(0, min(15, round((gpa / 100.0) * 15 + random.uniform(-1.5, 1.5)))))

    def hito_nota(gpa):
        return int(max(0, min(10, round((gpa / 100.0) * 10 + random.uniform(-1.5, 1.5)))))

    profile = AcademicProfile(
        user_id=user.id, course=course, scholarship_holder=scholarship,
        tuition_fees_up_to_date=tuition_ok, current_semester=semester,
        units_approved=units_approved, current_gpa=gpa, age_at_enrollment=age,
        gender=gender,
    ...
    record = AcademicRecord(
        user_id=user.id, gpa=gpa, enrolled_credits=semester * 20,
        failed_classes=failed,
        hito2_procesual=profile.hito2_procesual, hito2_nota=profile.hito2_nota,
        hito3_procesual=profile.hito3_procesual, hito3_nota=profile.hito3_nota,
        hito4_procesual=profile.hito4_procesual, hito4_nota=profile.hito4_nota,
        hito5_procesual=profile.hito5_procesual, hito5_nota=profile.hito5_nota,
    )
    db.add(record)
    ...
    for sub_name in selected_subjects:
        ...
        grade = AcademicSubjectGrade(
            user_id=user.id, subject_name=sub_name,
    ...
    for wave in range(3):
        ...
        db.add(AssessmentResponse(
            user_id=user.id, assessment_id=pss.id, answers=pss_answers,
            total_score=float(pss_score), risk_level=risk_label,
            share_with_psychologist=True, created_at=created
        ))
        db.add(AssessmentResponse(
            user_id=user.id, assessment_id=gad.id, answers=gad_answers,
            total_score=float(gad_score), risk_level=risk_label,
            share_with_psychologist=True, created_at=created
        ))
        db.add(AssessmentResponse(
            user_id=user.id, assessment_id=phq.id, answers=phq_answers,
            total_score=float(phq_score), risk_level=risk_label,
            share_with_psychologist=True, created_at=created
        ))

    # Risk Summary
    db.add(RiskSummary(
        user_id=user.id, current_risk_level=risk_label,
        prediction_confidence=round(random.uniform(0.82, 0.98), 4),
        dropout_probability=float(dropout_prob), dropout_risk=risk_label,
    ...
    for day in range(30):
        ...
        db.add(EmotionalCheckin(
            user_id=user.id, mood_score=mood, energy_level=energy,
            sleep_hours=sleep, academic_pressure=pressure,
...
def main():
    ...
    try:
        ...
        old = db.query(User).filter(User.email.like("student_%@estudiante.mentalink.edu")).all()
        if old:
            old_ids = tuple(u.id for u in old)
            tables = [
                "academic_subject_grades", "assessment_responses", "ai_predictions",
                "consents", "alerts", "academic_profiles", "academic_records",
                "appointments", "emotional_diary", "emotional_checkins", "risk_summaries",
            ]
            for t in tables:
                db.execute(text("DELETE FROM %s WHERE user_id IN :ids" % t), {"ids": old_ids})
            db.execute(text("DELETE FROM clinical_notes WHERE student_id IN :ids"), {"ids": old_ids})
            db.execute(text("DELETE FROM audit_logs WHERE actor_id IN :ids"), {"ids": old_ids})
        ...
        print("Distribucion: Low=%d, Medium=%d, High=%d" % (
            classifications.count("Low"), classifications.count("Medium"),
