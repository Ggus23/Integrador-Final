...
from app.models import User, UserRole, Consent, AcademicProfile, AcademicRecord, Assessment, AssessmentResponse, RiskSummary, EmotionalCheckin, AcademicSubjectGrade, EmotionalDiary
...
FEMALE_NAMES = [
    "Valeria Quispe", "Fernanda Flores", "Camila Mamani", "Gabriela Condori", "Natalia Rojas",
    "Andrea Vargas", "Sofia Castro", "Luciana Ramos", "Daniela Torrico", "Mariana Siles",
    "Claudia Mendoza", "Alejandra Ortiz", "Isabela Guzman", "Valentina Rocha", "Paulina Camacho",
    "Lucía Carvajal", "Elena Miranda", "Carmen Gutierrez", "Teresa Villarroel", "Patricia Suarez",
    "Adriana Pinto", "Beatriz Blanco", "Silvia Marin", "Gloria Herrera", "Alicia Medina"
]

MALE_NAMES = [
    "Alejandro Gómez", "Carlos Mamani", "Sebastian Flores", "Mateo Quispe", "Javier Condori",
    "Mauricio Rojas", "Rodrigo Vargas", "Daniel Castro", "Nicolas Ramos", "Gabriel Torrico",
    "Diego Siles", "Hugo Mendoza", "Adrian Ortiz", "Fernando Guzman", "Bruno Rocha",
    "Alvaro Camacho", "Andres Carvajal", "Jose Miranda", "Luis Gutierrez", "Ramiro Villarroel",
    "Mario Suarez", "Ricardo Pinto", "Jorge Blanco", "Walter Marin", "Oscar Herrera"
]
...
SUBJECTS_BY_COURSE = {
    "Ingeniería en Sistemas": [
        "Introducción a la Programación", "Cálculo I", "Álgebra Lineal", "Estructura de Datos", 
        "Base de Datos I", "Análisis y Diseño de Sistemas", "Sistemas Operativos", "Física I"
    ],
    "Psicología": [
        "Introducción a la Psicología", "Neuroanatomía", "Psicología del Desarrollo", "Psicobiología",
        "Psicología Clínica", "Psicopatología I", "Métodos de Investigación", "Procesos Cognitivos"
    ],
    "Medicina": [
...
def generate_pss_answers(target_score: int) -> dict:
    ...
    for item in items:
        ...
        val = answers[item]
        if item in reverse_items:
            if diff > 0 and val > 0: # Necesitamos subir el score, o sea bajar el valor real
                decrement = min(diff, val)
                answers[item] -= decrement
                diff -= decrement
            elif diff < 0 and val < 4: # Necesitamos bajar el score, o sea subir el valor real
                increment = min(-diff, 4 - val)
                answers[item] += increment
                diff += increment
        else:
            if diff > 0 and val < 4: # Necesitamos subir el score, o sea subir el valor real
                increment = min(diff, 4 - val)
                answers[item] += increment
                diff -= increment
            elif diff < 0 and val > 0: # Necesitamos bajar el score, o sea bajar el valor real
                decrement = min(-diff, val)
                answers[item] -= decrement
                diff += decrement
...
def generate_users():
    ...
    try:
        ...
        old_users = db.query(User).filter(User.email.like("student_real_%@unifranz.edu.bo")).all()
        if old_users:
            old_ids = [u.id for u in old_users]
            db.execute(text("DELETE FROM academic_subject_grades WHERE user_id IN :ids"), {"ids": tuple(old_ids)})
            db.execute(text("DELETE FROM assessment_responses WHERE user_id IN :ids"), {"ids": tuple(old_ids)})
            db.execute(text("DELETE FROM ai_predictions WHERE user_id IN :ids"), {"ids": tuple(old_ids)})
            db.execute(text("DELETE FROM consents WHERE user_id IN :ids"), {"ids": tuple(old_ids)})
            db.execute(text("DELETE FROM alerts WHERE user_id IN :ids"), {"ids": tuple(old_ids)})
            db.execute(text("DELETE FROM academic_profiles WHERE user_id IN :ids"), {"ids": tuple(old_ids)})
            db.execute(text("DELETE FROM academic_records WHERE user_id IN :ids"), {"ids": tuple(old_ids)})
            db.execute(text("DELETE FROM appointments WHERE user_id IN :ids OR psychologist_id IN :ids"), {"ids": tuple(old_ids)})
            db.execute(text("DELETE FROM emotional_diary WHERE user_id IN :ids"), {"ids": tuple(old_ids)})
            db.execute(text("DELETE FROM emotional_checkins WHERE user_id IN :ids"), {"ids": tuple(old_ids)})
            db.execute(text("DELETE FROM risk_summaries WHERE user_id IN :ids"), {"ids": tuple(old_ids)})
            db.execute(text("DELETE FROM users WHERE id IN :ids"), {"ids": tuple(old_ids)})
            db.commit()
            print(f"✅ Se eliminaron {len(old_users)} usuarios antiguos de prueba reales.")
        ...
        for idx in range(50):
            ...
            email_prefix = full_name.lower().replace(" ", ".").replace("á", "a").replace("é", "e").replace("í", "i").replace("ó", "o").replace("ú", "u").replace("ñ", "n")
            ...
            if risk == "Low":
                ...
                tuition_up_to_date = random.choice([True, True, True, False]) # 75% al día
            ...
            for sub_name in selected_subjects:
                ...
                subject_grade = AcademicSubjectGrade(
                    ...
                    hito2_procesual=min(15.0, (sub_gpa / 100.0) * 15.0 + random.uniform(-0.5, 0.5)),
                    hito2_nota=min(10.0, (sub_gpa / 100.0) * 10.0 + random.uniform(-0.5, 0.5)),
                    hito3_procesual=min(15.0, (sub_gpa / 100.0) * 15.0 + random.uniform(-0.5, 0.5)),
                    hito3_nota=min(10.0, (sub_gpa / 100.0) * 10.0 + random.uniform(-0.5, 0.5)),
                    hito4_procesual=min(15.0, (sub_gpa / 100.0) * 15.0 + random.uniform(-0.5, 0.5)),
                    hito4_nota=min(10.0, (sub_gpa / 100.0) * 10.0 + random.uniform(-0.5, 0.5)),
                    hito5_procesual=min(15.0, (sub_gpa / 100.0) * 15.0 + random.uniform(-0.5, 0.5)),
                    hito5_nota=min(10.0, (sub_gpa / 100.0) * 10.0 + random.uniform(-0.5, 0.5)),
                )
        ...
        print(f"   - Bajo (Low): {sum(1 for c in classifications if c == 'Low')} estudiantes")
        print(f"   - Medio (Medium): {sum(1 for c in classifications if c == 'Medium')} estudiantes")
        print(f"   - Alto (High): {sum(1 for c in classifications if c == 'High')} estudiantes")
...
if __name__ == "__main__":
    print("🚀 Iniciando generación de 50 usuarios reales para pruebas de clasificación...")
    generate_users()
