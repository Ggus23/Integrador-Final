    ...
    try:
        with engine.connect() as conn:
            with conn.begin():
                ...
                conn.execute(
                    text(
                        "UPDATE users SET email = 'admin@unifranz.edu.bo' WHERE email = 'admin@unifranz.test'"
                    )
                )
                ...
                if existing:
                    print(
                        f"⚠️ Admin ya existe: {email}. Actualizando contraseña a Admin123!..."
                    )
                    conn.execute(
                        text(
                            "UPDATE users SET hashed_password = :hashed_password, role = :role, is_active = :is_active WHERE id = :id"
                        ),
                        {
