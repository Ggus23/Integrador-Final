...
from app.models import User, UserRole, Assessment, AssessmentResponse, AcademicSubjectGrade
...
def generate_test_data():
    ...
    try:
        ...
        user_ids = [r[0] for r in db.execute(text("SELECT id FROM users WHERE email LIKE 'student_test_%' OR email LIKE '%.test'")).fetchall()]
        if user_ids:
            # Eliminar de tablas hijas usando SQL crudo para evitar problemas con modelos ORM desactualizados
            db.execute(text("DELETE FROM academic_subject_grades WHERE user_id IN :ids"), {"ids": tuple(user_ids)})
            db.execute(text("DELETE FROM assessment_responses WHERE user_id IN :ids"), {"ids": tuple(user_ids)})
            db.execute(text("DELETE FROM ai_predictions WHERE user_id IN :ids"), {"ids": tuple(user_ids)})
            db.execute(text("DELETE FROM consents WHERE user_id IN :ids"), {"ids": tuple(user_ids)})
            db.execute(text("DELETE FROM alerts WHERE user_id IN :ids"), {"ids": tuple(user_ids)})
            db.execute(text("DELETE FROM academic_profiles WHERE user_id IN :ids"), {"ids": tuple(user_ids)})
            db.execute(text("DELETE FROM academic_records WHERE user_id IN :ids"), {"ids": tuple(user_ids)})
            db.execute(text("DELETE FROM appointments WHERE user_id IN :ids OR psychologist_id IN :ids"), {"ids": tuple(user_ids)})
            db.execute(text("DELETE FROM emotional_diary WHERE user_id IN :ids"), {"ids": tuple(user_ids)})
            db.execute(text("DELETE FROM emotional_checkins WHERE user_id IN :ids"), {"ids": tuple(user_ids)})
            db.execute(text("DELETE FROM audit_logs WHERE actor_id IN :ids"), {"ids": tuple(user_ids)})
        ...
        subjects = [
            ...
            "Programación", "Biología", "Inglés", "Filosofía", "Arte"
        ]
        ...
        for i in range(1, 101):
            ...
            risk_level = "low" if pss_score < 14 else "medium" if pss_score < 27 else "high"
            ...
            assessment_response = AssessmentResponse(
                ...
                dropout_probability=min(1.0, (pss_score / 40) * 0.8),  # Mayor estrés = mayor riesgo
