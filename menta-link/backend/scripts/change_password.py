...
def main():
    ...
    parser.add_argument(
        "--email", type=str, default="admin@unifranz.edu.bo", help="User email address"
    )
    parser.add_argument(
        "--password",
        type=str,
        help="New password (optional, a random secure password will be generated if not provided)",
    )
    ...
    try:
        with engine.connect() as conn:
            with conn.begin():
                ...
                if not user:
                    print(
                        f"⚠️ User '{email}' not found. Let's check if another admin email exists..."
                    )
                    # Let's list some users so the admin knows what emails are available
                    users_result = conn.execute(
                        text("SELECT email, role FROM users LIMIT 10")
                    )
                ...
                conn.execute(
                    text(
                        "UPDATE users SET hashed_password = :hashed_password, must_change_password = :must_change_password WHERE id = :id"
                    ),
                    {
