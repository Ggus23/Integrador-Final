        ...
        summaries.append(
            {
                ...
                "phone_number": student.phone_number if current_user.role in [models.user.UserRole.PSYCHOLOGIST, models.user.UserRole.ADMIN] else None,
            }
        )
...
@router.get(
    "/{student_id}/history", response_model=List[schemas.student.StudentHistoryItem]
)
def read_student_history(
    ...
    """
    Obtener el historial emocional y de tests consolidado de un estudiante.
    """
    from app.services.student_history_service import student_history_service

    return student_history_service.get_combined_history(db, student_id)


@router.get("/{student_id}/trends", response_model=schemas.student.StudentTrends)
def read_student_trends(
    student_id: int,
    db: Session = Depends(deps.get_db),
    current_user: models.user.User = Depends(deps.get_staff_user),
) -> Any:
    """
    Obtener tendencias emocionales y cálculo de ARI (Academic Risk Index).
    """
    from app.services.emotional_trends_service import emotional_trends_service

    dist = emotional_trends_service.get_emotion_distribution(db, student_id)
    evolution = emotional_trends_service.get_weekly_evolution(db, student_id)
    ari = emotional_trends_service.calculate_ari(db, student_id)

    return {
        "distribution": dist,
        "weekly_evolution": evolution,
        "ari_score": ari["ari_score"],
        "ari_level": ari["risk_level"],
    }
