...
def verify_user_phone_manually(
    ...
    """Confirma el teléfono después de que el psicólogo contactó al estudiante."""
    user = db.query(models.user.User).filter(models.user.User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="Usuario no encontrado")
    ...
    user.phone_review_note = "Confirmado después de contacto telefónico."
    user.phone_approved_by_id = None
    user.phone_approved_at = None
    db.add(user)
    db.commit()
    db.refresh(user)
    return user
