...
async def check_all_reminders(db: Session, force_all: bool = False):
    ...
    """
    users = db.query(User).filter(User.expo_push_token != None).all()
    assessments = db.query(Assessment).all()
    today = date.today()
    current_hour = datetime.now().hour
    
    messages_to_send = []
    print(f"🧠 Procesando reglas (Hora actual: {current_hour}:00) para {len(users)} usuarios...")
    ...
    for user in users:
        ...
        if force_all or (8 <= current_hour <= 11):
            ...
            has_checkin = db.query(EmotionalCheckin).filter(
        ...
        if force_all or (13 <= current_hour <= 17):
            for assessment in assessments:
                last_resp = db.query(AssessmentResponse).filter(
                ...
                if force_all or not last_resp or (datetime.now() - last_resp.created_at.replace(tzinfo=None)).days >= 15:
                    messages_to_send.append({
                        "to": user.expo_push_token,
        ...
        if force_all or (19 <= current_hour <= 23):
            has_diary_today = db.query(EmotionalDiary).filter(
    ...
    # Envío final por lotes
    if messages_to_send:
        print(f"🚀 Enviando lote total de {len(messages_to_send)} notificaciones...")
        from app.services.notifications import send_push_notifications
        await send_push_notifications(messages_to_send)
        print("✅ Todo enviado.")
    else:
