...
class RiskService:
    ...
    def calculate_dropout_risk(
        emotional_risk_score: int, failed_classes: int, gpa_drop: bool
    ) -> bool:
        """
        Calculates if the user is at high risk of academic dropout based on
        emotional and academic variables.
        Formula: (Persistencia de Score Emocional Crítico) + (Materias Reprobadas > 2) + (Descenso en GPA)
        """
        critical_emotion = emotional_risk_score >= 8
        academic_struggle = failed_classes > 2 or gpa_drop

        if critical_emotion and academic_struggle:
            return True
        return False
