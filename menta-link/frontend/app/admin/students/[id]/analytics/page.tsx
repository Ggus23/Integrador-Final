...
export default function StudentAnalyticsPage() {
  ...
  const fetchStudent = async () => {
    try {
      ...
      setStudent({ id: studentId, full_name: details.full_name, email: details.email, role: details.role });
    ...
    return (
      <Layout>
        <div className="mx-auto max-w-7xl space-y-8 px-4 pb-12">
          <Skeleton className="h-10 w-64 bg-slate-800 rounded" />
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[1, 2, 3].map(i => <Skeleton key={i} className="h-28 bg-slate-800 rounded-xl" />)}
          </div>
          <Skeleton className="h-[300px] bg-slate-800 rounded-xl w-full" />
          <Skeleton className="h-[250px] bg-slate-800 rounded-xl w-full" />
        </div>
      </Layout>
    );
  }
    ...
    <Layout>
      <div className="mx-auto max-w-7xl space-y-8 px-4 pb-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-foreground font-serif text-3xl font-bold">Analítica del Estudiante</h1>
            </div>
        ...
        {data.loading ? (
          ...
          <Card className="bg-rose-500/10 border-rose-500/30 text-rose-400 p-6 text-center">
          ...
          <div className="space-y-6 text-slate-100">
              ...
              {(() => {
                ...
                return (
                  <div className={`rounded-xl border p-5 space-y-2 ${
                    ...
                    <p className="text-xs font-black uppercase tracking-widest text-slate-500">Estado de Riesgo</p>
              ...
              {(() => {
                ...
                return (
                  <div className={`rounded-xl border p-5 space-y-2 ${
                    ...
                    <p className="text-xs font-black uppercase tracking-widest text-slate-500">Prob. Abandono (IA)</p>
                    <p className={`text-3xl font-black ${
                      isHigh ? 'text-rose-400' : isMed ? 'text-amber-400' : 'text-emerald-400'
                    }`}>
                    ...
                    <p className="text-sm text-slate-400">Basado en GPA, reprobación e inactividad.</p>
                  </div>
                );
              })()}
              ...
              {(() => {
                ...
                return (
                  <div className="rounded-xl border border-slate-700/40 bg-slate-800/30 p-5 space-y-2">
                    <p className="text-xs font-black uppercase tracking-widest text-slate-500">Confianza del Modelo</p>
                    <p className="text-3xl font-black text-purple-400">{pct !== null ? `${pct.toFixed(0)}%` : '—'}</p>
                    {pct !== null && (
                      <div className="w-full bg-slate-800 rounded-full h-2">
                        <div className="h-2 rounded-full bg-purple-400" style={{ width: `${Math.min(pct, 100)}%` }} />
                      </div>
                    )}
            ...
            <Card className="border-slate-700/40 bg-slate-900/60 p-6">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-4">
                <div>
                  ...
                  <p className="text-sm text-slate-400 mt-1">Variables con mayor peso en el diagnóstico. Barra más larga = mayor impacto.</p>
                </div>
                <span className="text-xs bg-teal-500/10 text-teal-400 font-bold px-3 py-1.5 rounded border border-teal-500/20 shrink-0">
            ...
            <Card className="border-slate-700/40 bg-slate-900/60 p-6 sm:p-8">
              <div className="border-b border-slate-700/50 pb-4 mb-6">
                ...
                <p className="text-sm text-slate-400 mt-1">Índice de equilibrio, distribución de emociones y evolución semanal.</p>
              </div>
            ...
            {details.academic_profile && (
              <Card className="border-slate-700/40 bg-slate-900/60 p-6 space-y-6">
                <h4 className="text-base font-bold text-white">🎓 Rendimiento Académico</h4>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="bg-slate-800/60 border border-slate-700/40 rounded-xl p-4">
                    <p className="text-xs font-black uppercase text-slate-500">Carrera</p>
                    <p className="text-base font-bold mt-1">{details.academic_profile.course || 'No especificada'}</p>
                  </div>
                  <div className="bg-slate-800/60 border border-slate-700/40 rounded-xl p-4">
                    <p className="text-xs font-black uppercase text-slate-500">Materias Aprobadas</p>
                    <p className="text-base font-bold mt-1">{details.academic_profile.units_approved} unidades</p>
                  </div>
                  <div className="bg-purple-950/30 border border-purple-700/30 rounded-xl p-4">
                    <p className="text-xs font-black uppercase text-purple-400/70">GPA Total</p>
                    ...
                    <div className="w-full bg-slate-700/50 rounded-full h-2 mt-2">
                      <div className="h-2 rounded-full bg-purple-400" style={{ width: `${Math.min(details.academic_profile.current_gpa, 100)}%` }} />
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  {[2, 3, 4, 5].map((num) => {
                    ...
                    return (
                      <div key={num} className="bg-slate-800/50 border border-slate-700/40 rounded-xl p-4 text-center">
                        <p className="text-xs font-black uppercase text-slate-500 mb-1">Hito {num}</p>
                        ...
                        <div className="w-full bg-slate-700/50 rounded-full h-1.5 mt-2">
                          <div className={`h-1.5 rounded-full ${bar}`} style={{ width: `${Math.min(total, 100)}%` }} />
                        </div>
                      </div>
                    );
                  })}
          ...
          <Card className="border-slate-700/40 bg-slate-900/60 p-12 text-center">
            <p className="text-slate-500 italic text-lg">No hay datos analíticos para mostrar.</p>
          </Card>
        )}
      </div>
    </Layout>
