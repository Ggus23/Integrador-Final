...
const CustomPieTooltip = ({ active, payload, total }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-card/97 border-border border rounded-2xl p-4 shadow-xl backdrop-blur-md animate-in fade-in zoom-in-95 duration-150">
        <div className="flex items-center gap-2 mb-2">
          <span className="text-2xl">{meta.emoji}</span>
          <p className="text-foreground text-sm font-black capitalize">{d.name}</p>
        </div>
        <div className="space-y-1 text-xs text-muted-foreground">
          <p>Registros: <span className="text-foreground font-bold">{d.value}</span></p>
          <p>Proporción: <span className="text-foreground font-bold">{percentage}%</span></p>
        </div>
      </div>
    );
  }
...
const getAriConfig = (level: string, score: number) => {
  if (level === 'Alto Riesgo' || score < 0.35)
    return { color: '#ef4444', bg: 'bg-rose-500/10', border: 'border-rose-500/20', label: 'Requiere atención urgente', badge: 'bg-rose-500', icon: TrendingDown };
  if (level === 'Riesgo Medio' || score < 0.65)
    return { color: '#f59e0b', bg: 'bg-amber-500/10', border: 'border-amber-500/20', label: 'Atención moderada', badge: 'bg-amber-500', icon: Minus };
  return { color: '#10b981', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', label: 'En equilibrio', badge: 'bg-emerald-500', icon: TrendingUp };
};
...
export function EmotionalTrendsPanel({ data }: EmotionalTrendsPanelProps) {
  ...
  return (
    <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
      ...
      <Card className={`border-t-4 bg-card/30 border-border/40 backdrop-blur-md shadow-2xl transition-shadow hover:shadow-xl md:col-span-1 ${ariConfig.border}`}
      ...
      >
        ...
        <CardContent className="flex flex-col items-center pt-2 pb-4 gap-4">
          {/* Gauge SVG */}
          <div className="relative flex items-center justify-center">
            <svg className="h-36 w-36 -rotate-90 transform" viewBox="0 0 128 128">
              <circle cx="64" cy="64" r={radius} stroke="currentColor" strokeWidth="10" fill="transparent" className="text-muted/15" />
              <circle
            ...
            <div className="absolute flex flex-col items-center">
              ...
              <span className="text-[9px] text-muted-foreground uppercase tracking-wider font-bold">Equilibrio</span>
            </div>
          </div>

          {/* Badge + description */}
          <Badge className={`px-4 py-1.5 text-sm font-bold text-white flex items-center gap-1.5 ${ariConfig.badge}`}>
      ...
      <Card className="shadow-2xl md:col-span-2 border-border/40 bg-card/30 backdrop-blur-md">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {pieData.map((entry) => {
              const pct = totalEmotions > 0 ? ((entry.value / totalEmotions) * 100).toFixed(0) : '0';
              const meta = EMOTION_META[entry.rawName];
              return (
                <div key={entry.rawName} className="flex items-center gap-2 bg-muted/20 border border-border/20 rounded-lg px-2.5 py-1.5">
                  <span className="text-base">{meta?.emoji || '🙂'}</span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-bold text-foreground capitalize truncate">{entry.name}</span>
                      <span className="text-[10px] font-black ml-1" style={{ color: entry.color }}>{pct}%</span>
                    </div>
                    <div className="w-full bg-muted/40 rounded-full h-1 mt-1">
                      <div className="h-1 rounded-full" style={{ width: `${pct}%`, backgroundColor: entry.color }} />
                    </div>
                  </div>
                </div>
              );
