...
def read_academic_profile_me(
    ...
    """
    profile = db.query(models.AcademicProfile).filter(models.AcademicProfile.user_id == current_user.id).first()
    if not profile:
        # Create an empty profile if it doesn't exist
        profile = models.AcademicProfile(
            user_id=current_user.id,
            hito2_procesual=0.0, hito2_nota=0.0,
            hito3_procesual=0.0, hito3_nota=0.0,
            hito4_procesual=0.0, hito4_nota=0.0,
            hito5_procesual=0.0, hito5_nota=0.0
        )
        db.add(profile)
        db.commit()
        db.refresh(profile)
...
def read_academic_profile(
    ...
    """
    profile = db.query(models.AcademicProfile).filter(models.AcademicProfile.user_id == student_id).first()
    if not profile:
