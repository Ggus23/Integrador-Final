...
class User(Base):
    ...
    consent = relationship(
        "Consent", back_populates="user", uselist=False, cascade="all, delete-orphan"
    )
    assessment_responses = relationship(
        "AssessmentResponse", back_populates="user", cascade="all, delete-orphan"
    )
    emotional_checkins = relationship(
        "EmotionalCheckin", back_populates="user", cascade="all, delete-orphan"
    )
    risk_summary = relationship(
        "RiskSummary",
        back_populates="user",
        uselist=False,
    ...
    clinical_notes_received = relationship(
        "ClinicalNote",
        foreign_keys="[ClinicalNote.student_id]",
